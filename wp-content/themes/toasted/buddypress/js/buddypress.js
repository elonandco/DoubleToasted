/* Buddypress Core JS */
/*  Reset Buddypress Terms */
var BP_DTheme = {newest:"OOooooOOOOO Somebody just posted something new!",accepted:"Accepted", close:"Close", comments:"comments", leave_group_confirm:"Are you sure you want to leave this group?", mark_as_fav:" ", my_favs:"My Favorites", rejected:"Rejected", remove_fav:" ", show_all:"Show All", show_all_comments:"Show all comments for this thread", show_x_comments:"Show all %d comments", unsaved_changed:"Your profile has unsaved changes. If you leave the page, the changes will be lost.", view:"View"};
...
jq(document).ready( function() {
	...
	jq("#aw-whats-new-submit").on( 'click', function() {
		...
		if ( firstrow.length ) {
			...
			// * By Pound
			// If we're on a profile page set the timestamp
			// this is a workaround for the post_update action retrieving a list of posts
			// without the current context and updating them in the feed
			if (jq(this).hasClass('dt-bp-button-home')) {
				timestamp = 1407333278;
			}
		}
		...
		function(response) {
			...
			// Return share box to original state
			jq('#whats-new-form-home').removeClass('active');
			jq('.activity-greeting').slideDown(200);
			jq('#home-post-update').slideUp(400);
			jq('.rtmedia-container').fadeOut();
			jq('#whats-new').val('');
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				act_wrap = jq("#activity-stream");

				if ( jq('body').hasClass('bp-user') && !jq('body').hasClass('my-account') ) { // profile wall above recent activity
					jq('.activity-profile-mentions').prepend(response); // we only want the respone prepended to the first activity stream
				} else {
					jq(".activity-list").not('.activity-profile-mentions').prepend(response);
				}
	...
	jq('#activity-filter-select select').change( function() {
		...
		// if (filter == 'news_feed' ||
		// 	filter == 'friend_updates' ||
		// 	filter == 'group_updates' ||
		// 	filter == 'user_mentions' || 
		// 	filter == 'user_favorites' ) {
		// 	var scope = 'news-feed';
		// }
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			target.stop(); // stop any other attached events
			
			var count = parseInt(target.attr('count'));
			...
			if (type == 'fav') {
				...
				target.removeClass('fav').addClass('unfav');	
				target.text( (count) + ' Toasted');
				...
				count = count - 1;
				target.addClass('fav').removeClass('unfav').attr('count', count).text( (count) + ' Toast');
		...
		if ( target.parent().hasClass('load-more') ) {
			...
			// Added by Pound for user Dash
			// Is this the user dash? If it is then we need to use a different ajax function that calls the activity-loop-dash template.
			var actionajax = 'activity_get_older_updates'; // set default action
			var checkloaddash = jq('#dt-load-userdash'); // is the load element on the user dash?
			if (checkloaddash.length > 0) { 
				actionajax = 'activity_get_older_userdash'; // set new action if true
			}
			...
			function(response)
			{
				...
				jq("#buddypress ul.activity-list").not('.activity-profile-mentions').append(response.contents);
				jq('body').trigger('bpActivityLoaded');
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('acomment-reply') || target.parent().hasClass('acomment-reply') || target.hasClass('single-acomment-reply')) {
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] == '-1' ) {
					...
					form.fadeOut( 200, function() {
						...
						response = response.replace("animate-post", "animate-post visible"); /* Otherwise post will be invisible */
						var the_comment = jq.trim( response ); /* Preceeding whitespace breaks output with jQuery 1.9.0 */
		...
		if ( target.hasClass('acomment-delete') || target.hasClass('single-acomment-delete') ) {

			var link_href = target.attr('href');
			var comment_li = target.parent().parent().parent();
			var form = comment_li.parents('div.activity-comments').children('form');
			var parent_id = comment_li.parents('#activity-stream > li').data('parent-id');
			...
			/* Counts children of deleted comment so we can re-calculate total replies */
			...
			/* Subtract children count from original reply count */
			...
			function(response) {
				...
				if ( response[0] + response[1] == '-1' ) {
					...
					comment_li.slideUp(200, function() {
						comment_li.remove();
					});
...
// 	});
	...
	jq('#buddypress').on( 'click', function(event) {
		...
		if ( target.parent().parent().hasClass('pagination') && !target.parent().parent().hasClass('no-ajax') ) {
			...
			var page_number = target.attr('href').split( 'page=' ); // @dev added page to split arg for search results fix
	...
	// This checks each new activity as it's dynamically updated against the latest heartbeat activity.
	...
	// Adds "Show All Comments" to Community and Profile Pages
	is_community_home = jq('.community-home');

	if (is_community_home.length > 0) {
	
		/* @mod - Replace "Show All Comments" on Community Homepage */
		jq('.community-home #activity-stream .activity-comments li').hide();
		jq('.community-home #activity-stream .show-all').remove();
		jq('.community-home #activity-stream .activity-comments ul').prepend('<li class="community-show-all"><a href="#activity-64270/show-all/" title="Show all comments for this thread">Show all comments</a></li>');
		
		jq('.community-home .activity-comments').delegate('.community-show-all','click',function() {
			jq(this).hide();
			jq(this).parent().children('.community-home .activity-comments li').not('.community-show-all').slideDown(200);
		});
		...
		if ( caller == 'pag-bottom' && jq('#subnav').length ) {
			...
			jq('html,body').animate({scrollTop: top.offset().top}, 'slow', function() {
				...
				jq(target).fadeOut( 100, function() {
					...
					jq(this).first().fadeIn(100, function(){
						jq("body").trigger('gridloaded');						
					});
					
					// Mod for List/Grid View
					if ( object == 'groups' ) {
						newpag = jq('.new-pagination-groups').html();
						jq('.first-group-pagination').html(newpag);
					}
...
/* jQuery Cookie plugin - WooCommerce already Loading This */
