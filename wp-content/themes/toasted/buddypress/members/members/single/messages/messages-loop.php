  ...
  <ul class="mesage-item animated animate-when-almost-visible bottom-to-top">
  	<li id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
		...
		<?php 
			...
			// Mod: Makes it so unread count does not show up if it doesnt exist
			$unreadCount = bp_get_message_thread_unread_count();
			if ($unreadCount) {
				echo '<div class="thread-count"><span class="unread-count">' . $unreadCount . '</span></div>';
			}
			
		?>
		
		<p id="dt-msg-subject"><a href="<?php bp_message_thread_view_link(); ?>" title="<?php _e( "View Message", "buddypress" ); ?>"><?php bp_message_thread_subject(); ?></a></p>
      ...
      <?php if ( 'sentbox' != bp_current_action() ) : ?>
        ...
        <div class="thread-info">
					<p class="thread-excerpt"><a href="<?php bp_message_thread_view_link(); ?>" title="<?php _e( "View Message", "buddypress" ); ?>"><?php bp_message_thread_excerpt(); ?></a></p>
				</div>
