	...
	function polldaddy_shortcode( $atts ) {
		...
		if ( intval( $rating ) > 0 && !$no_script ) { //rating embed		
			...
			if ( is_ssl() )
				...
				$rating_js_file = "http://kcoolman.staging.wpengine.com/wp-content/themes/toasted/js/custom-rating.js"; // todo: jameson, remove this when going live
	...
	function generate_scripts() {
		...
		if ( is_array( self::$scripts ) ) {
			if ( is_ssl() )
				...
				$rating_js_file = "/wp-content/themes/toasted/js/custom-rating.js"; // todo: jameson, remove this when going live
				//$rating_js_file = "http://i0.poll.fm/js/rating/rating.js";
