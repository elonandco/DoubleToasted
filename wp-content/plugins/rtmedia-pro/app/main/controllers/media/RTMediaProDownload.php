...
class RTMediaProDownload extends RTMediaUserInteraction {
    ...
    /*
     * Checks if download button is allowed for current media type from the rtemdia settings
     */
    function check_disable(){
		if( isset( $this->media ) && isset( $this->media->media_type ) ){
			global $rtmedia;
			$options = $rtmedia->options;
			$media_type = $this->media->media_type;

			if( ! ( isset( $options['allowedTypes_' . $media_type . '_download'] ) && ( $options['allowedTypes_' . $media_type . '_download'] == '1' ) ) ){
				return true;
			}
		}
		return false;
