...
class RTMediaAlbumList {
	...
	function rtm_edit_wp_album_gallery() {
		...
		$content .= '<h3 class="rtmedia-attachment-display-none">' . __( 'Attachment Details', 'rtmedia' ) . '</h3>';
		$content .= '<div class="rtmedia-attachment-delete-success">' . __( 'Media Deleted Successfully.', 'rtmedia' ) . '</div>';
		$content .= '<div class="rtmedia-attachment-save-success">' . __( 'Media Updated Successfully.', 'rtmedia' ) . '</div>';
		$content .= '<label class="setting rtmedia-attachment-display-none">';
		$content .= '<span>' . __( 'Title', 'rtmedia' ) . '</span>';
		$content .= '<input type="text" id="rtmedia_attachment_title" />';
		$content .= '</label>';
		$content .= '<label class="setting rtmedia-attachment-display-none">';
		$content .= '<span>' . __( 'Description', 'rtmedia' ) . '</span>';
		$content .= '<textarea id="rtmedia_attachment_description"></textarea>';
		$content .= '</label>';
		$content .= '<div class="clear"></div>';
		$content .= '<div id="rtmedia_attachment_button_div">';
		$content .= '<a class="rtmedia-attachment-display-none rtmedia-delete-attachment button-primary button-danger button-large">' . __( 'Delete Permanently', 'rtmedia' ) . '</a>';
		$content .= '<a href="#" class="rtmedia-attachment-display-none button media-button button-primary button-large save-button">' . __( 'Save', 'rtmedia' ) . '</a>';
		$content .= '</div>';
		$content .= '</div>';
		$content .= '</div>';
		$content .= '</div>';
	...
	function add_meta_box() {
		...
		<div id="wp-content-media-buttons" class="wp-media-buttons" style="float: right;">
			<a href="#" id="rtmedia-edit-wp-gallery-button" class="button add_media" data-editor="content" title="Edit Media">
				<span class="wp-media-buttons-icon"></span><?php _e( 'Edit Media', 'rtmedia' ); ?>
			</a>
		</div>
		<div class="clear"></div>
	...
	function rtmedia_pro_add_settings_tab( $sub_tabs ) {
		$sub_tabs[ ] = array( 'href' => '#rtmedia-wp-albums', 'icon' => 'rtmicon-picture-o', 'title' => __( 'rtMedia WordPress Albums', 'rtmedia' ), 'name' => __( 'WordPress Albums', 'rtmedia' ), 'callback' => array( 'RTMediaAlbumList', 'wp_albums_content' ) );
	...
	function wp_albums_content( $render_options, $options ) {
		$render_options[ 'rtmedia_enable_wp_album' ] = array( 'title' => __( 'Enable sitewide gallery section', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'checkbox' ), 'args' => array( 'key' => 'rtmedia_enable_wp_album', 'value' => $options[ 'rtmedia_enable_wp_album' ], 'desc' => __( 'This will enable sitewide gallery feature. Gallery is collection of albums and can be manage from WordPress dashboard, like a custom post type.', 'rtmedia' ) ), 'group' => "80" );
		$create_gallery_page_url = get_bloginfo( 'url' ) . '/wp-admin/edit.php?post_type=rtmedia_album';
		$render_options[ 'rtmedia_wp_album_slug' ] = array( 'title' => __( 'Slug for sitewide gallery section', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'textbox' ), 'args' => array( 'key' => 'rtmedia_wp_album_slug', 'value' => $options[ 'rtmedia_wp_album_slug' ], 'desc' => __( 'It is not recommended to change this setting again and again. This will be act as a base for all albums created from dashboard.', 'rtmedia' ) ), 'after_content' => __( 'You can', 'rtmedia' ) . ' <a href=\'' . $create_gallery_page_url . '\'>' . __( 'manage sitewide galleries from here', 'rtmedia' ) . '.</a>', 'group' => "80" );
