...
class RTMediaProFavList {
	...
	function edit_favlists_media_list( $media_type ) {
		if ( isset( $media_type ) && $media_type == "favlist" ){
			...
			if ( $media_list != "" ){
				...
				$media_table = "<div class='rtmedia-favlist-media-table'>" . "<table id='" . $rtmedia_media->id . "' class='rtmedia-favlist-media-list'>" . "<thead><tr><th>" . __( 'Title', 'rtmedia' ) . "</th><th colspan='2'>" . __( 'Actions', 'rtmedia' ) . "</th></tr></thead>" . "<tbody>";
				$count       = 1;
				foreach ( $media_list as $media_id ) {
					...
					if ( $media ){
						...
						$list .= "<tr>" . "<td class='rtm-edit-media-list-title'>" . $count ++ . ". <a href='" . $link . "' target='_blank' title='" . __( 'View this media', 'rtmedia' ) . "' >" . $media[ 0 ]->media_title . "</a></td>" . "<td class='rtm-edit-media-list-edit'><a href='" . $link . "edit' target='_blank' title='" . __( 'Edit this media', 'rtmedia' ) . "' ><i class='rtmicon-edit rtmicon-fw'></i>" . __( 'Edit', 'rtmedia' ) . "</a></td>" . "<td class='rtm-edit-media-list-delete'><span id='" . $media[ 0 ]->id . "' class='rtmedia-remove-media-from-favlist' title='" . __( 'Remove media from this favlist', 'rtmedia' ) . "' ><i class='rtmicon-times-circle' ></i>" . __( 'Remove', 'rtmedia' ) . "</span></td>" . "</tr>";
					}
	...
	function modify_media_query( $media_query, $action_query, $query ) {
		...
		if ( is_array( $media_query ) && isset( $media_query[ 'media_type' ] ) && $media_query[ 'media_type' ] == 'favlist' ){
            // If action is delete then no need to unset favlist_id
            if( ( $action_query->action == "delete" ) ) {
                add_filter( 'rtmedia_before_delete_media_redirect', array( $this, 'rtmedia_before_delete_media_redirect_favlist' ), 10, 1 );
	...
	function general_content_add_favlist_group( $general_group ) {
		$general_group[ 25 ] = __( "User's Favorite Media List", 'rtmedia' );
    ...
    function rtmedia_before_delete_media_redirect_favlist( $redirect_url ) {
        return $redirect_url . RTMEDIA_FAVLIST_SLUG;
    }
}