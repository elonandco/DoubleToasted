...
class RTMediaProFavListInteraction extends RTMediaUserInteraction {
	...
	function get_user_favlist_form() {
		...
		if ( $profile_favlists != "" ){
			...
			$list = '<form action="' . $link . '" name="" method="post" class="rtm-add-to-favlist-form">' . '<span class="rtm-favlist-span" id="rtm-favlist-select"><label for="rtm_favlist_id">' . __( 'Select FavList: ', 'rtmedia' ) . '</label><select name="rtm_favlist_id" id="rtm-favlist-list" class="">' . '<option value="0">' . __( 'Select FavList', 'rtmedia' ) . '</option>' . "<option value='-1'>" . __( 'Create FavList', 'rtmedia' ) . "</option>";
