...
class RTMediaProPoints {
	...
	?>
		...
		<h3 class="hndle"><span><?php _e( 'Reward Point Settings', 'rtmedia' ); ?></span></h3>
	    </div>
	    <div class="row">
		...
		<div class="columns large-12">
		    ...
		    ?>
		    <p><?php echo __('Active', 'rtmedia') . " CubePoints " . __('for', 'rtmedia') . " rtMedia" ; ?> <a href="<?php echo get_admin_url(); ?>admin.php?page=cp_admin_modules#rtmedia-cp" target ="_blank"><?php _e('here', 'rtmedia'); ?></a>.</p>
		    <?php
			    }
		    ?>
		    <?php
			    if( cp_module_activated( 'rtmedia' ) ) {
		    ?>
				<p><?php _e('Set reward points', 'rtmedia'); ?> <a href="<?php echo get_admin_url(); ?>admin.php?page=cp_admin_config#rtmedia-cp" target="_blank"><?php _e('here', 'rtmedia'); ?></a>.</p>
		    <?php
			    }
		    ?>
				</fieldset>
