...
class RTMediaProModeration extends RTMediaUserInteraction {
	...
	function __construct() {
		...
		$args = array( 'action' => 'moderate', 'label' => __( 'Report', 'rtmedia' ), 'undo_label' => __( 'Withdraw Report', 'rtmedia' ), 'privacy' => 20, 'countable' => true, 'undoable' => true, 'icon_class' => 'rtmicon-warning rtmicon-fw', );
	...
	function moderate_allow_media() {
		if ( isset( $_POST[ 'media_id' ] ) && $_POST[ 'media_id' ] != "" ) {
			...
			$data = array( "privacy" => $prev_privacy, );
	...
	function moderate_page() {
		...
		echo "<h2>" . __( 'rtMedia: Media moderation', 'rtmedia' ) . "</h2>";
	...
	function rtmedia_pro_add_moderation_tab( $sub_tabs ) {
		$sub_tabs[ ] = array( 'href' => '#rtmedia-moderation', 'icon' => 'rtmicon-eraser', 'title' => __( 'rtMedia Moderation', 'rtmedia' ), 'name' => __( 'Moderation', 'rtmedia' ), 'callback' => array( 'RTMediaProModeration', 'moderation_content' ) );
		return $sub_tabs;
	}
	...
	static function moderation_content() {
		...
		$render_options[ 'moderation_enableModeration' ] = array( 'title' => __( 'Moderation', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'checkbox' ), 'args' => array( 'key' => 'moderation_enableModeration', 'value' => $options[ 'moderation_enableModeration' ], 'desc' => __( 'Enable moderation in rtMedia', 'rtmedia' ) ) );
		$render_options[ 'moderation_removeContentAfterReports' ] = array( 'title' => __( 'Remove content after reports', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'number' ), 'args' => array( 'key' => 'moderation_removeContentAfterReports', 'value' => $options[ 'moderation_removeContentAfterReports' ], 'desc' => __( 'Remove content automatically after specified number of reports in rtMedia', 'rtmedia' ), 'class' => array( 'rtmedia-setting-text-box' ) ) );
		$render_options[ 'moderation_adminEmails' ] = array( 'title' => __( 'Admin emails (comma separated)', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'textarea' ), 'args' => array( 'key' => 'moderation_adminEmails', 'value' => ( isset( $options[ 'moderation_adminEmails' ] ) && $options[ 'moderation_adminEmails' ] != "" ) ? $options[ 'moderation_adminEmails' ] : get_site_option( 'admin_email' ), 'desc' => __( 'Send emails to admins automatically after specified number of reports in rtMedia', 'rtmedia' ), 'class' => array( 'rtmedia-setting-text-area' ) ) );
		$moderation_page_url = admin_url( 'admin.php?page=rtmedia-moderate' );
		$render_options[ 'moderation_emailNotificationFreq' ] = array( 'title' => __( 'Email notification frequency', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'selectBox' ), 'args' => array( 'key' => 'moderation_emailNotificationFreq', 'default' => $options[ 'moderation_emailNotificationFreq' ], 'desc' => __( 'Email notification frequency after specified number of reports in rtMedia', 'rtmedia' ), 'class' => array( 'rtmedia-setting-select-box' ), 'selects' => array( "daily" => "Daily", "hourly" => "Hourly", "instant" => "Instant" ) ), 'after_content' => __( 'You can <a href=\'' . $moderation_page_url . '\'>manage moderated medias from here</a>', 'rtmedia' ), );
		?>
		<div class="postbox metabox-holder">
			<h3 class="hndle"><span><?php _e( 'Moderation Settings', 'rtmedia' ); ?></span></h3>
		</div>
		<?php
	...
	function check_take_down_contet( $media_id, $action ) {
		...
		$rtmediainteraction = new RTMediaInteractionModel();
		$columns = array( 'media_id' => $media_id, 'action' => $action );
	...
	function process() {
		// return if moderation is disabled
		if ( $this->check_disable() ) {
			return true;
		}
		...
		$user_id = $this->interactor;
		$media_id = $this->action_query->id;
		// check if user has already moderated this media or not
		$check_action = $rtmediainteraction->check( $user_id, $media_id, $action );
		$return = array();
		$return[ "next" ] = "";
		$return[ "rt_redirect" ] = "";
		// if moderated
		if ( $check_action ) {
			$where = array( 'user_id' => $user_id, 'media_id' => $media_id, 'action' => $action );
			// check if media is under moderation or not
			if ( $this->check_take_down_contet( $media_id, $action ) ) {
				$prev_privacy = get_rtmedia_meta( $media_id, "moderate-privacy" );
				$rtmediamedia = new RTMediaMedia();
				$data = array( "privacy" => $prev_privacy, );
				//restore original privacy of media
			...
			// make user's action entry in database
	...
	function rtmedia_moderate_media( $user_id, $media_id, $action, $media_post_id, $privacy, $value = "Media moderated" ) {
		...
		if ( $this->check_take_down_contet( $media_id, $action ) ) {
			...
			$rtmediamedia = new RTMediaMedia();
			$data = array( "privacy" => "80", );

			/**
			 * as we are updating privacy to 80, first update activity privacy and then update media privacy
			 * otherwise non admin user won't be able to get moderated media to update activity privacy
			 */
			$this->update_rtmedia_activity_privacy( $media_id, 80 );
	...
	function update_rtmedia_activity_privacy( $id, $new_privacy = false ) {
		...
		if ( class_exists( 'RTMediaActivityModel' ) ) {
			...
			if( $new_privacy !== false && $max_privacy < $new_privacy ){
				$max_privacy = $new_privacy;
			}
