...
class RTMediaProBBPress {
	...
	function rtmedia_bbpress_content() {
		...
		$radios                                              = array();
		$radios[ 'thumb_image' ]                             = "<strong>" . __( 'With Thumbnail + file name', 'rtmedia' ) . "</strong>";
		$radios[ 'thumb_title' ]                             = "<strong>" . __( 'Without Thumbnail (only file name)', 'rtmedia' ) . "</strong>";
		...
		<div class="postbox metabox-holder">
			<h3 class="hndle"><span><?php _e( 'Attachment Support for bbPress', 'rtmedia' ); ?></span></h3>
			<?php
				...
				<div class="row section">
					...
					<div class="columns large-6">
						...
						<span data-tooltip class="has-tip" title="<?php echo ( isset( $option[ 'args' ][ 'desc' ] ) ) ? $option[ 'args' ][ 'desc' ] : "NA"; ?>">
							<i class="rtmicon-info-circle rtmicon-fw"></i>
						</span>
					</div>
	...
	public function save_reply( $reply_id, $topic_id, $forum_id, $anonymous_data, $reply_author ) {
		...
		if ( isset( $_FILES[ 'rtmedia_file_multiple' ] ) && isset( $_FILES[ 'rtmedia_file_multiple' ][ 'name' ] ) && isset( $_FILES[ 'rtmedia_file_multiple' ][ 'name' ][ 0 ] ) && $_FILES[ 'rtmedia_file_multiple' ][ 'name' ][ 0 ] != '' ){
			if ( isset( $_FILES[ 'rtmedia_file_multiple' ] ) && sizeof( $_FILES[ 'rtmedia_file_multiple' ] ) > 0 ){
				...
				remove_action( 'rtmedia_after_add_media', array( $this, 'rtmedia_after_add_media' ), 10, 3 );
			}
		}
	}
