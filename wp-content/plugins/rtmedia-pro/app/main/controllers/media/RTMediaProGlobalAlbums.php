...
class RTMediaProGlobalAlbums {
	...
	function delete_global_album() {
		...
		// do not allow to delete default selected album
		if ( $default_album_id == $delete_album_id ) {
			echo "0";
			die();
		}

		// update album_id of media who has album_id set to delete-album_id and then delete album from database
		$rtmediamodel = new RTMediaModel();
		...
		// update global album options
		$global_albums = rtmedia_get_site_option( 'rtmedia-global-albums' );
		$new_default_global_album_index = array_search( $delete_album_id, $global_albums );
		if( $new_default_global_album_index !== false && isset( $global_albums[ $new_default_global_album_index ] ) ){
			unset( $global_albums[ $new_default_global_album_index ] );
		}
		rtmedia_update_site_option( 'rtmedia-global-albums', $global_albums );
		echo "1";
		die();
	...
	public static function global_albums_content() {
		...
		<div class="global-album-settings">
			<div class="postbox metabox-holder">
				<h3 class="hndle"><span><?php _e( 'Default Albums', 'rtmedia' ); ?></span></h3>
			</div>
			<div class="row">
			...
			<div class="row">
				<div class="columns large-12">
					...
					<table class="default-global-albums">
						<thead>
						<tr>
							<th><?php _e( 'Album name', 'rtmedia' ); ?></th>
							<th colspan="2"><?php _e( 'Actions', 'rtmedia' ); ?></th>
							<th><?php _e( 'Preselected album', 'rtmedia' ); ?></th>
						</tr>
						</thead>
						<tbody>
						<?php
			...
			<div class="postbox metabox-holder">
				<h3 class="hndle"><span><?php _e( 'Add Default Album', 'rtmedia' ); ?></span></h3>
			</div>
			<div class="row">
				<div class="columns large-12">
