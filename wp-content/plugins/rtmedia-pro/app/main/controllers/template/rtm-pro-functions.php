	...
	if ( $display === true ){
		...
		<div id="rtmedia-media-options">
			<a href="#new-playlist-modal" class="rtmedia-modal-link"
			   title="<?php _e( "Add Playlist", "rtmedia" ); ?>"><i
					class="rtmicon-plus-circle rtmicon-fw"></i><?php _e( "Add Playlist", "rtmedia" ); ?></a>
		</div>
		<div id="new-playlist-modal" class="rtmedia-popup mfp-hide">
			<div id="rtm-modal-container">
				<h2 class="rtm-modal-title"><?php _e( "Create Playlist", "rtmedia" ); ?></h2>

				<p>
					<label for='rtmedia_playlist_name'><?php _e( 'Title :', 'rtmedia' ); ?></label>
					<input type="text" id="rtmedia_playlist_name" value="" class='rtm-input-medium'/>
				</p>
...
function get_playlist_media_list_table() {
	...
	if ( $table ){
		...
		echo "<i class='rtmicon-attention rtmicon-fw'></i>" . __( ' Currently there are no media in this playlist.' );
	}
}
...
function add_album_privacy( $screen = false ) { //$screen for identifying from where the do_action is applied
	...
	if ( $rtmediaprivacy ){
		if ( $screen == 'album-edit' ){
			...
			echo "<p><label class='rtm-modal-grid-title-column' for='rtmedia_select_album_privacy'>" . __( 'Privacy : ', 'rtmedia' ) . "</label>" . $rtmediaprivacy . "</p>";
		}
	}
}
...
function rtmedia_pro_album_activity_privacy( $sql, $select_sql, $from_sql, $where_sql, $sort, $pag_sql = '' ) {
	...
	if( $activity_upgrade_done ){
		...
		$from_sql  = " FROM {$bp->activity->table_name} a LEFT JOIN {$wpdb->users} u ON a.user_id = u.ID LEFT JOIN {$rtmedia_model->table_name} m ON ( a.id = m.activity_id and m.blog_id = '" . get_current_blog_id() . "' ) LEFT JOIN {$rtmedia_model->table_name} ap ON ( ap.id = m.album_id and ap.blog_id = '" . get_current_blog_id() . "' ) LEFT JOIN {$rtmedia_activity_model->table_name} ra ON ( a.id = ra.activity_id and ra.blog_id = '".  get_current_blog_id()."' ) ";
...
function rtmedia_view_original_button( $media_id = false ) {
	...
	if ( is_user_logged_in() && isset( $options[ 'allowedTypes_' . $media[ 0 ]->media_type . '_download' ] ) && $options[ 'allowedTypes_' . $media[ 0 ]->media_type . '_download' ] == "1" ){
		$url    = wp_get_attachment_url( rtmedia_media_id( $media_id ) );
		$button = "";
		if ( $url ){
			$button = "<form action='" . $url . "' method='link'>";
			$button .= '<button type="submit" class="rtmedia-action-buttons button rtmedia-view-original"><i class="rtmicon-eye rtmicon-fw"></i><span>' . __( 'View Original', 'rtmedia' ) . '</span></button>';
			$button .= "</form>";
			echo $button;
		}
	}
			...
			if ( $rtmedia_backbone[ 'backbone' ] ){
				...
				if( is_rt_admin() || ( isset( $rtmedia_media ) && isset( $rtmedia_media->media_author ) && $rtmedia_media->media_author == get_current_user_id() ) ){
					...
					<div class='rtmedia-gallery-item-actions'>
						...
						<a href="#" class="no-popup rtmp-delete-media" title='<?php _e( 'Delete this media', 'rtmedia' ); ?>'>
							<i class='rtmicon-trash-o rtmicon-fw'></i><?php _e( 'Delete', 'rtmedia' ); ?>
						</a>
					</div>
				<?php
				}
...
function rtmedia_add_album_button_for_non_logged( $options ) {
	if ( ! is_user_logged_in() ){
		...
		if ( isset( $rtmedia->options[ 'general_enableAlbums' ] ) && $rtmedia->options[ 'general_enableAlbums' ] == 1 && isset( $rtmedia->options[ 'general_enableCreateAlbums' ] ) && $rtmedia->options[ 'general_enableCreateAlbums' ] == 1 ){
			$options[ ] = '<span><a href="#rtmedia-login-register-modal" class="rtmedia-modal-link" title="' . __( 'Add new Album', 'rtmedia' ) . '" id="rtmedia-login-register-modal"><i class="rtmicon-plus-circle rtmicon-fw"></i>' . __( 'Add Album' ) . '</span> ';
		}
	}
...
function rtmedia_add_upload_album_button() {
	if ( ! is_user_logged_in() ){
		echo '<span><a href="#rtmedia-login-register-modal" class="primary rtmedia-upload-media-link rtmedia-modal-link" id="rtmedia-login-register-modal" title="' . __( 'Upload Media', 'rtmedia' ) . '"><i class="rtmicon-upload rtmicon-fw"></i>' . __( 'Upload' ) . '</a></span>';
	}
}
...
function rtmedia_media_custom_thumbnail_title( $media_type ) {
	...
	if ( ! in_array( $media_type, $allow_custom_thumb_media ) ){
		?>
		<dd><a href="#custom_thumb"><i class="rtmicon-picture-o rtmicon-fw"></i><?php _e( 'Media thumbnail', 'rtmedia' ); ?></a>
		</dd>
	<?php
	}
}
...
add_action( 'init', 'rtmedia_check_audio_tags_option' );

function rtmedia_check_audio_tags_option() {
	global $rtmedia;
	$options = $rtmedia->options;
	if ( isset( $options[ 'general_enable_audio_tags' ] ) && $options[ 'general_enable_audio_tags' ] == "1" ) {
		if ( class_exists( "RTMediaTags" ) ) {
			add_action( 'rtmedia_add_edit_tab_content', 'rtmedia_audio_editor_content', 12, 1 );
			add_action( 'rtmedia_add_edit_tab_title', 'rtmedia_audio_tag_editor_title', 12, 1 );
		}
	}	
}

function rtmedia_audio_editor_content( $type = 'music' ) {
	$media_id = rtmedia_media_id();
	$media_type = rtmedia_type();
	$media_ext = rtmedia_media_ext();
	if ( isset( $media_type ) && $media_type == 'music' && $type == 'music' && $media_ext = 'mp3' )  {
		$filepath = get_attached_file( $media_id );
		
		$tag_title = array(
			'Title' => __( 'Title','rtmedia' ),
			'Artist' => __( 'Artist', 'rtmedia' ),
			'Album' => __( 'Album', 'rtmedia' ),
			'Year' => __( 'Year', 'rtmedia') ,
			'Genre' => __( 'Genre', 'rtmedia' ),
			'Comment' => __( 'Comment','rtmedia' ),
		);
		echo '<div class="content" id="panel2">';
		$tag_class = new RTMediaTags( $filepath );
		foreach ( $tag_title as $title => $value ) {
			$title_id = strtolower( $title );
			$tag_value = $tag_class->$title_id;
			echo '<div class="rtmedia-edit-tag"><label>' . $value . ':</label><input type="text" name="rtmedia-edit-tag-'. $title_id .'"  value="' . $tag_value. '"/></div>';
		}
		
		echo '<div class="rtmedia-edit-tag"><label>' . __( 'Track', 'rtmedia' ). ':</label><div id="rtmedia-tag-track" ><input type="text" name="rtmedia-edit-tag-track"  value="'.$tag_class->track.'"/> of <input  type="text" name="rtmedia-edit-tag-track-total"  value="'.$tag_class->track_total.'"/></div></div>';
		echo '</div>';
	}
}

function rtmedia_audio_tag_editor_title( $type = 'music' ) {
	$media_type = rtmedia_type();
	$media_ext = rtmedia_media_ext();
	
	if ( isset( $media_type ) && $media_type == 'music' && $type == 'music' && $media_ext == 'mp3' ){
		echo '<dd><a href="#panel2" class="rtmedia-modify-image"><i class="rtmicon-tags rtmicon-fw"></i>' . __( "Set tags", "rtmedia" ) . '</a></dd>';
	}
}

function rtmedia_save_audio_tag( $id, $state ) {
	global $rtmedia;
	$options = $rtmedia->options;
	...
	if ( isset( $options[ 'general_enable_audio_tags' ] ) && $options[ 'general_enable_audio_tags' ] == "1" ) {
		if ( class_exists( "RTMediaTags" ) ) {
			if ( $state ) {
				if ( ( $media_type == 'music' ) && ( $media_ext = 'mp3' ) ) {
					$filepath = get_attached_file( rtmedia_media_id( $id ) );
					$tag_title = array( 'title', 'artist', 'album', 'year', 'genre', 'comment' );
					$post_val = array();
					foreach ( $tag_title as $tag ) {
						$post_val[ $tag ] = $_POST[ 'rtmedia-edit-tag-' .$tag ];
					}
					$media_tags = new RTMediaTags( $filepath );
					foreach( $post_val as $key => $val ) { 	
						$media_tags->$key = $val;
					}
					$track = $_POST[ 'rtmedia-edit-tag-track' ];
					$track_total = $_POST[ 'rtmedia-edit-tag-track-total' ];
				
					$track_info = '';
					if ( !empty( $track ) ) {
						$track_info = $track .( !empty( $track_total )  ? '/'. $track_total : '' );
					
					}
				
					$media_tags->track = $track_info;
					$media_tags->save();
					$post_val['track'] = $track_info;
				
					update_rtmedia_meta( $id, 'audio_tags', $post_val);
				}
			}
		}
	}	
...
add_action( 'rtmedia_after_edit_media', 'rtmedia_save_audio_tag', 9, 2 );
		...
		if ( ! isset( $options[ 'allowedTypes_photo_upload_limit' ] ) || ( isset( $options[ 'allowedTypes_photo_upload_limit' ] ) && ( $options[ 'allowedTypes_photo_upload_limit' ] <= 0 || ( ( $_FILES[ 'rtmedia_media_custom_thumbnail' ][ 'size' ] / ( 1024 * 1024 ) ) <= $options[ 'allowedTypes_photo_upload_limit' ] ) ) && in_array( $image_extn, $photo_allowed_types ) ) ){
			...
			if ( ( $media_type == 'music' ) && ( $media_ext = 'mp3' ) ) {
				if ( class_exists( "RTMediaTags" ) ) {	
					...
					$media_tag->set_art( file_get_contents( $thumb_file['url'] ), $image_extn, $_FILES[ 'rtmedia_media_custom_thumbnail' ][ 'name' ] );
					$media_tag->save();
				}
			}
		} else {
...
function rtmedia_display_audio_tags(){
	global $rtmedia;
	$options = $rtmedia->options;
	$media_type = rtmedia_type();
	$media_ext = rtmedia_media_ext();
	...
	if ( isset( $options[ 'general_enable_audio_tags' ] ) && $options[ 'general_enable_audio_tags' ] == "1" ) {
		if ( class_exists( "RTMediaTags" ) ) {	
			if ( $media_type == 'music' && $media_ext == 'mp3'  ) {
				$media_tags = get_rtmedia_meta( $media_id, 'audio_tags' );
				if ( $media_tags == false ) {
					$filepath = get_attached_file( rtmedia_media_id() );
					$media_tags = new RTMediaTags( $filepath );
					...
					$tag_title = array(
						...
						'genre' => __( 'Genre', 'rtmedia' ),
						'comment' => __( 'Comment','rtmedia' ),
						'track' => __( 'Track', 'rtmedia' ),
					);
			
					$tag_data = array();
					foreach( $tag_title as $tag => $tag_label ){
						$value = $media_tags->$tag;
						$tag_data[ $tag ] = $value;
						if ( empty($value) ){
							continue;
						}
						$html .= "<span class='rtmedia-media-tag'><label>".$tag_label.":</label>&nbsp;". $value."</span>";
					}
					add_rtmedia_meta( $media_id, 'audio_tags', $tag_data );
				} else {
					foreach( $media_tags as $tag => $value ) {
						if ( empty( $value ) ){
							continue;
						}
						$html .= "<span class='rtmedia-media-tag'><label>".$tag.":</label>&nbsp;". $value."</span>";
					}
				}
				if( $html != '' ){
					$html = "<div class='rtmedia-media-tags-head'>". __( 'Audio Tags', 'rtmedia' ) ."</div> <div class='rtmedia-media-tags'>" . $html . "</div>";
				}
				echo $html;
			}
		}	
...
add_action('rtmedia_actions_before_description', 'rtmedia_display_audio_tags', 99999 );
...
// generic function to check if bulk edit is allowed or not
function rtmedia_is_bulk_edit_allowed(){
	global $rtmedia_query;
	if ( rtmp_is_music_tab() && is_music_playlist_view_enabled() ){
		return false;
	}
	...
	$allow = false;
	$user_id = get_current_user_id();
	if(  isset( $rtmedia_query->query ) && ( $rtmedia_query->query['context'] == 'profile' || $rtmedia_query->query['context'] == 'group' ) ){
		if( $rtmedia_query->query['context'] == 'profile' && isset( $rtmedia_query->media_query[ 'media_author' ] ) && $user_id == $rtmedia_query->media_query[ 'media_author' ] ){
			$allow = true;
		}
...
function rtmedia_bulk_edit_option( $options ) {
	if ( rtmedia_is_bulk_edit_allowed() ){
		$options[ ] = '<a href="#" class="rtmedia-bulk-edit" title="' . __( 'Bulk edit media', 'rtmedia' ) . '"><i class="rtmicon-edit rtmicon-fw"></i>' . __( 'Bulk Edit', 'rtmedia' ) . '</a>';
	}
	return $options;
}

// close <form> opened for bulk edit
...
function rtmedia_bulk_edit_container() {
	...
	if ( rtmedia_is_bulk_edit_allowed() ){
		...
		<div class="rtmedia-bulk-edit-options">
			...
			<button type="button" class="rtmedia-bulk-move" title="<?php _e( 'Move selected media to album', 'rtmedia' ); ?>"><?php _e( 'Move', 'rtmedia' ); ?></button>
			<button type="button" class="rtmedia-bulk-delete-selected rtm-alert-btn" title="<?php _e( 'Delete the selected media', 'rtmedia' ); ?>"><?php _e( 'Delete', 'rtmedia' ); ?></button>
			<?php if ( is_rtmedia_privacy_enable() && is_rtmedia_privacy_user_overide() && $context == 'profile' ){ ?>
				<button type="button" class="rtmedia-change-privacy" title="<?php _e( 'Change the privacy of the selected Media', 'rtmedia' ); ?>">
					<?php _e( 'Change Privacy' ); ?>
				</button>
			<?php } ?>

			<?php wp_nonce_field( 'rtmedia_bulk_delete_nonce', 'rtmedia_bulk_delete_nonce' ); ?>
			<?php RTMediaMedia::media_nonce_generator( $rtmedia_query->query[ 'context_id' ] ); ?>
		...
		<div class="rtmedia-bulk-move-container rtm-dashed-border">
			...
			<?php
				...
				$options = '';
				if( $context == 'profile' ){
					$album_options = rtmedia_user_album_list();
				} else if( $context == 'group' ){
					$album_options = rtmedia_group_album_list();
				} else {
					$album_options = '';
				}
				if ( $album_options ){
		...
		<?php if ( is_rtmedia_privacy_enable() && is_rtmedia_privacy_user_overide() && $context == 'profile' ){ ?>
			<div class="rtmedia-bulk-privacy-container rtm-dashed-border">
				...
				?>
				<input type="button" class="rtmedia-change-privacy-selected" name="change-privacy-selected" value="<?php _e( 'Save', 'rtmedia' ); ?>"/>
			</div>
		<?php } ?>
...
function rtm_url_parser_callback(){
    require_once RTMEDIA_PRO_PATH . 'lib/url-scraper-php/website_parser.php';
    ...
    if(  class_exists( 'WebsiteParser' ) ){
		...
		$body = wp_remote_get( esc_url_raw( $url ) );
...
add_action( 'rtmedia_favlist_gallery_actions', 'rtm_create_favlist_option', 99 );
...
?>
	<div id="rtmedia-media-options">
		<a href="#new-favlist-modal" class="rtmedia-modal-link" title="<?php _e( "Add FavList", "rtmedia" ); ?>">
			<i class="rtmicon-plus-circle rtmicon-fw"></i><?php _e( "Add FavList", "rtmedia" ); ?>
		</a>
	</div>
	<div id="new-favlist-modal" class="rtmedia-popup mfp-hide">
		<div id="rtm-modal-container">
			<h2 class="rtm-modal-title"><?php _e( "Create FavList", "rtmedia" ); ?></h2>
			<p>
...
add_action( 'rtmedia_media_gallery_actions', 'rtmedia_favlist_options', 80 );

function rtmedia_favlist_options() {
    $options_start = $options_end = $option_buttons = $output = "";
	$options       = array();
	$options       = apply_filters( 'rtmedia_favlist_actions', $options );
	if ( ! empty( $options ) ){

		$options_start = '<span class="click-nav" id="rtm-media-options-list">
                <span class="no-js">
                <span class="clicker rtmedia-action-buttons"><i class="rtmicon-cog rtmicon-fw"></i>' . __( 'Options', 'rtmedia' ) . '</span>
...
add_filter( 'rtmedia_favlist_actions', 'rtmedia_favlist_actions', 11 );

function rtmedia_favlist_actions( $options ) {
    if ( !is_rtmedia_favlist() || ! is_user_logged_in() ){
		return;
	}
    ...
    if( isset( $media_query_clone_favlist ) && isset( $media_query_clone_favlist[ 'media_type' ] ) ) {
        if( is_rt_admin() || ( isset( $media_query_clone_favlist[ 'media_author' ] ) && get_current_user_id() == $media_query_clone_favlist[ 'media_author' ] ) ) {
            $options[ ] = "<a href='edit/' class='rtmedia-edit' title='" . __( 'Edit Favlist', 'rtmedia' ) . "' ><i class='rtmicon-edit rtmicon-fw'></i>" . __( 'Edit Favlist', 'rtmedia' ) . "</a>";
			$options[ ] = '<form method="post" class="favlist-delete-form rtmedia-inline" action="delete/">' . wp_nonce_field( 'rtmedia_' . $media_query_clone_favlist[ 'id' ], 'rtmedia_media_nonce' ) . '<button type="submit" name="favlist-delete" class="rtmedia-delete-favlist" title="' . __( 'Delete Favlist', 'rtmedia' ) . '"><i class="rtmicon-trash-o rtmicon-fw"></i>' . __( 'Delete Favlist', 'rtmedia' ) . '</button></form>';
        }
    }
...
function get_favlist_media_list_table() {
	...
	if ( $table ){
		...
		echo "<i class='rtmicon-attention rtmicon-fw'></i>" . __( ' Currently there isn\'t any media in this FavList.' );
	}
}
