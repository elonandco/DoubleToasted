...
class RTMediaProAdmin {
	...
	function rtmedia_version_notice() {
		if( defined( 'RTMEDIA_VERSION' ) && RTMEDIA_VERSION < '3.6.13' ) {
			?>
				<div class="error">
					<p><?php _e( 'This version of rtMedia-PRO requires rtMedia version 3.6.13 or higher. Please update rtMedia.', 'rtmedia'); ?></p>
				</div>
			<?php
		}
	}
	...
	?>
		<span style="float: right"><?php _e( '0 = Unlimited', 'rtmedia'); ?></span>
	<?php
	}
	...
	function rtmedia_display_content_groups( $general_group ) {
		$general_group[ 20 ] = __( 'Miscellaneous', 'rtmedia' );

		return $general_group;
	}

	function general_content_groups( $general_group ) {
		$general_group[ 20 ] = __( 'Playlist Feature', 'rtmedia' );
	...
	function add_media_type_settings_before_heading() {
		?>
		<div class="row"> <div class="columns large-7 medium-7 small-7"> <div class="row">
	<?php
	}
		...
		<div class="row">
			<div class="columns large-6 medium-6 small-6">
				<h4 data-tooltip class="has-tip" title="<?php echo __( "Limit file size for media.", "rtmedia" ); ?>">
					<abbr><?php echo __( "Upload Limit(MB)", "rtmedia" ); ?></abbr>
				</h4>
			</div>
			<div class="columns large-6 medium-6 small-6">
				<h4 data-tooltip class="has-tip" title="<?php echo __( "Show download button for media types.", "rtmedia" ); ?>">
					<abbr><?php echo __( "Allow Download", "rtmedia" ); ?></abbr>
				</h4>
			</div>
		</div>
	</div>
		...
		<div class="columns large-5 medium-5 small-5">
			<div class="row">
				...
				<?php
					...
					$args = array(
						'key' => 'allowedTypes_' . $key . '_upload_limit',
						'value' => $section[ 'upload' ],
				...
				<div class="columns large-6 medium-6 small-6">
				<?php
//					if ( ! isset( $section[ 'download' ] ) ){
//						$section[ 'download' ] = 0;
//					}
					$args = array(
						'key' => 'allowedTypes_' . $key . '_download',
						'value' => $section[ 'download' ],
						'class' => array( 'rtmedia-setting-text-box' ),
					);
					RTMediaFormHandler::checkbox( $args );
				?>
				</div>
			</div>
	...
	function rtmedia_general_content_add_default_value( $defaults ) {
		...
		$defaults[ 'general_enable_audio_tags' ]           	    = 0;
		$defaults[ 'allowedTypes_photo_download' ]           	= 0;
		$defaults[ 'allowedTypes_music_download' ]           	= 0;
		$defaults[ 'allowedTypes_video_download' ]           	= 0;
	...
	function rtmedia_album_control_setting( $render_options, $options ) {
		$render_options[ 'general_enableCreateAlbums' ] = array(
			'title'    => __( 'Allow user to create albums', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'checkbox' ), 'args' => array(
				'key' => 'general_enableCreateAlbums', 'value' => $options[ 'general_enableCreateAlbums' ], 'desc' => __( 'You can either allow user to create albums themselves using this setting and/or provide list of default albums.', 'rtmedia' ), 'class' => array( 'rtmedia-album-setting' )
			), 'group' => "20"
		);
	...
	function rtmedia_general_content_single_view_options( $render_options, $options ) {
		...
		$render_options[ 'general_enable_audio_tags' ]               = array(
			'title' => __( 'Display Tags for audio ( mp3 ) files.', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'checkbox' ), 'args' => array(
				'key' => 'general_enable_audio_tags', 'value' => $options[ 'general_enable_audio_tags' ], 'desc' => __( 'Enabling this option,will display the audio tags embedded inside audio( mp3 ) files. The tags are also editable. ', 'rtmedia' )
			)
		);
