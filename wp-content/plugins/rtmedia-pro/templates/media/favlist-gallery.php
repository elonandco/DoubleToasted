<div class="rtmedia-container favlist">

	<?php do_action( 'rtmedia_before_favlist_gallery' );
		  $title = get_rtmedia_gallery_title(); ?>

	<div id="rtm-gallery-title-container" class="row">
		<h2 class="rtm-gallery-title columns large-5 small-12 medium-5"><?php if( $title ) { echo $title; }
                   else { _e( 'All Favlists', 'rtmedia' ) ; } ?></h2>
	...
	<?php if ( have_rtmedia() ){ ?>
		...
		<ul class="rtmedia-list rtmedia-album-list <?php echo rtmedia_media_gallery_class(); ?>">
			<?php while ( have_rtmedia() ) : rtmedia(); ?>
				<?php include( 'favlist-gallery-item.php' ); ?>
			<?php endwhile; ?>
		</ul>

		<div class='rtmedia_next_prev row'>
			<!--  these links will be handled by backbone later -- get request parameters will be removed  -->
			<?php
			//            $display = '' ;
			//            if ( rtmedia_offset () != 0 )
	...
	?>
		...
		<?php
			$message = __ ( "Sorry !! There's no FavList found for the request !!", "rtmedia" );
			echo apply_filters('rtmedia_no_favlist_found_message_filter', $message);
		?>
		</p>
	<?php
		}
	...
	<?php do_action( 'rtmedia_after_favlist_gallery' ); ?>
