function deleteArticlesNotification(action_id, item_id, adminUrl){
	...
	userID = jQuery("#articles-dir-list").attr("metaid");  

    jQuery.ajax({
        type: 'post',
        url: adminUrl,
        data: { action: "deleteArticlesNotification", action_id:action_id, item_id:item_id, user_id:userID },
        success:
        function(data) {
...
function savePost(){
    ...
    if(validateFields()){                      
        ...
        jQuery.ajax({
                type: 'post',
                url: MyAjax.ajaxurl,
                data: { action: actionName, post_title:postTitle, post_content:postContent, post_image:postImage, category_id:categoryId, tag_names:tagnames, status:status, post_id:postId, user_id:userID },                                                
                success:
                function(response) {   
...
function deleteArticle(postId,userID){
    jQuery("#delete-"+postId).addClass("deleting");          
	jQuery.ajax({
	            type: 'post',
	            url: MyAjax.ajaxurl,
	            data: { action: "delete_article", post_id:postId, user_id:userID },                                                
	            success:
	            function(response) {        
	               jQuery("#articles span").html(parseInt(jQuery("#articles span").html())-1);
